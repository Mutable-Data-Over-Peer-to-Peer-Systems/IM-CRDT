...
func main() {
	...
	// flag.Bool("updating", false, "do I update the data")
	...
	bootstrapPeer := flag.String("ni", "", "don't read standard input on this client")
	flag.Parse()
	if *mode == "BootStrap" {
		...
		Tests.Peer1Concu(*peerName, *updatesNB, *ntpServ, *encode) // ------------- MANAGE CONCURENCY !!!
		...
		if *updating {
			...
			Tests.Peer2ConcuUpdate(*peerName, *bootstrapPeer, *updatesNB, *ntpServ, *encode) // ------------- MANAGE CONCURENCY !!!
		} else {
			fmt.Println("NOT UPDATING FIOU")
			Tests.Peer2Concu(*peerName, *bootstrapPeer, *updatesNB, *ntpServ, *encode) // ------------- MANAGE CONCURENCY !!!
		}
