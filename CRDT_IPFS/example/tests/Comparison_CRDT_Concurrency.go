...
func Peer1Concu(peername string, nbUpdates int, ntpServ string, encode string) {
	...
	SetCrdt1 := Set.Create_CRDTSetOpBasedDag(sys1, peername, "", encode)
...
func Peer2Concu(peername string, bootStrapPeer string, nbUpdates int, ntpServ string, encode string) {
	sys1, err := IpfsLink.InitNode(peername, bootStrapPeer)
	if err != nil {
		panic(fmt.Errorf("Failed To instanciate IFPS & LibP2P clients : %s", err))
	}

	SetCrdt1 := Set.Create_CRDTSetOpBasedDag(sys1, peername, bootStrapPeer, encode)
	file, err := os.OpenFile(peername+"/time.csv", os.O_CREATE|os.O_WRONLY, 0755)
	file.WriteString("CID,time,time_retrieve,time_compute,time_add_IPFS\n")
	if err != nil {
		panic(fmt.Errorf("Error openning file file\nerror : %s", err))
...
func Peer2ConcuUpdate(peername string, bootStrapPeer string, nbUpdates int, ntpServ string, encode string) {
	sys1, err := IpfsLink.InitNode(peername, bootStrapPeer)
	if err != nil {
		panic(fmt.Errorf("Failed To instanciate IFPS & LibP2P clients : %s", err))
	}
