...
func GetIPFS(ipfs *IpfsLink, cids [][]byte) ([]files.Node, error) {
	...
	if len(cids) > 0 {
		...
		file, err = os.OpenFile("node1/timeConcurrentRetrieve.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0755)
		if err != nil {
	...
	for index, c := range cids {
		go func(i int) {
			str_CID := StrCids[i]
			files[i], err = ipfs.IpfsCore.Unixfs().Get(ipfs.Ctx, icorepath.IpfsPath(str_CID))
			if err != nil {
